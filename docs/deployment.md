...
python scripts/replay_deployment_proxy.py \
  --source local \
  ...
  --eval-end 2025-06-01 \
  --engine both \
  --output-dir runtime/replay_proxy_validation_live \
  --fail-on-mismatch
...
- `runtime/replay_proxy_validation_live/comparison.json`: parity for the validation window that produced the published SOLUSDT 1h stress metrics. This drives the live/demo deployment lifecycle from historical candles instead of using the broker feed.
...
docker compose --profile test run --rm live-path-validation-replay
...
- `live-path-validation-replay`: historical validation feed parity through the actual live/demo deployment lifecycle (`latest_signal_from_featured`, `DemoBroker.open_position`, `manage_position`, partial exits, trailing stops, final exits). It uses 2020-01-01 to 2025-06-01 for indicator warmup and scores only 2024-01-01 to 2025-06-01, matching the optimizer's 245-trade stress table.
